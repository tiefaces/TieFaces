...
public class DataHandler {
	...
	// public SRIForm buildRequestForSaving(String formid,Workbook wb,
	// Map<String, SheetConfiguration> sheetConfigMap, FormulaEvaluator
	// formulaEvaluator, DataFormatter dataFormatter) {
	//
	// }
	...
	private SheetConfiguration getsheetConfigByPageTypeId(
		...
		for (SheetConfiguration sheetConfig : sheetConfigMap.values()) {
			try {
				debug("Web Form FormDataHandler getFormPageTypeId = "
						+ sheetConfig.getFormPageTypeId() + " pageTypeId  = "
						+ pageTypeId);
				if (sheetConfig.getFormPageTypeId().equalsIgnoreCase(
						pageTypeId.toString()))
					return sheetConfig;
			} catch (Exception ex) {
				debug("Web Form FormDataHandler getsheetConfigByPageTypeId error = "
						+ ex.getLocalizedMessage());
				return null;
			}
		}
		debug("Web Form FormDataHandler getsheetConfigByPageTypeId cannot find sheetConfig in template with pageTypeId = "
				+ pageTypeId);
		return null;
	}

	// public void populateDataToExcelSheet(SRIForm formobj, Map<String,
	// SheetConfiguration> sheetConfigMap, boolean skipPageBody) {
	//
	// for (SRIFormPage formpage: formobj.getPages() ) {
	// SheetConfiguration sheetConfig =
	// getsheetConfigByPageTypeId(sheetConfigMap, formpage.getFormPageTypeId());
	// if (sheetConfig == null) {
	...
	// private void loadPageData(SheetConfiguration sheetConfig,SRIFormPage
	// formpage) {
	//
	//
	...
	// private void assembleExcelRow( int row, int topRow, Sheet sheet,
	// SheetConfiguration sheetConfig,FormulaEvaluator
	// formulaEvaluator,SRIFormPage formpage) {
	//
	// SRIFormDataRecord formdata = formpage.getDataRecords().get(row - topRow);
	//
	// for (Map.Entry<String, List<CellFormAttributes>> entry :
	// sheetConfig.getCellFormAttributes().entrySet()) {
	...
	// int datacol = Integer.parseInt(attrValue.substring(4)) - 1; // data array
	// is 0 based
	// cellValue = formdata.getData()[datacol];
	// } else if (attrValue.equalsIgnoreCase("NOTES")) {
	// if (row == topRow) cellValue = formpage.getNote(); // notes only set in
	// top row
	// else assemble = false;
	// }
	// if (assemble){
	...
	// String[]
	// rowcol=parent.getCellHelper().getRowColFromExcelReferenceName(targetCell);
	// if (!rowcol[1].isEmpty()) {
