...
public class SQLRunner {
	...
	Pattern selectPattern = Pattern.compile("^\\s*select\\s",
			Pattern.CASE_INSENSITIVE);
	Pattern plSQLStartPattern = Pattern.compile("^\\s*(declare|begin)\\s",
			Pattern.CASE_INSENSITIVE);
	Pattern plSQLEndPattern = Pattern.compile("^\\s*/\\s*$",
			Pattern.CASE_INSENSITIVE);
	...
	// public void openDatabaseConnection(boolean autoCommit) throws
	// NamingException,
	// SQLException
	// {
	// InitialContext initialContext = new InitialContext();
	// DataSource dataSource = (DataSource)
	// initialContext.lookup(this.dataSourceName);
	...
	public String runSQLs(String sourceSQL) throws Exception {
		...
		try {
			...
			if (getSqlExceptionThrown()) {
				debug("Exception throwed: " + results);
	...
	private ArrayList<String> generateSQL(String sqlSet) {
		...
		for (String sql : sqlList) {
			...
			if (plSQLStartMatcher.find()) {
				plSQLMode = true;
			} else if (plSQLEndMatcher.matches()) {
				plSQLMode = false;
			}
	...
	private String getResultSet(ResultSet resultSet) throws SQLException {
		...
		rows = getRows(resultSet, columns);

		results = formatResultSet(rows, columns);

		return results;
	}

	private ArrayList<Column> getColumnMetaData(ResultSet resultSet)
			throws SQLException {
		...
		for (int i = 1; i <= resultSetMetaData.getColumnCount(); ++i) {
			Column column = new Column(resultSetMetaData.getColumnName(i)
					.toLowerCase(), resultSetMetaData.getColumnType(i),
		...
		for (ArrayList<Object> row : rows) {
			for (int i = 0; i < row.size(); ++i) {
				...
				case java.sql.Types.BLOB:
					...
					unPrintable = new String(blob.getBytes((long) 1,
							(int) blob.length()));
					value = (row.get(i) == null) ? "" : unPrintable.replaceAll(
							"\\p{Cntrl}", "?");
					break;
				...
				case java.sql.Types.DATE:
					value = (row.get(i) == null) ? "" : dateFormat((Date) row
							.get(i));
					break;

				case java.sql.Types.NUMERIC:
					value = (row.get(i) == null) ? ""
							: numericFormat((BigDecimal) row.get(i));
					break;

				case java.sql.Types.TIME:
					value = (row.get(i) == null) ? "" : timeFormat((Time) row
							.get(i));
					break;

				case java.sql.Types.TIMESTAMP:
					value = (row.get(i) == null) ? ""
							: timeStampFormat((Timestamp) row.get(i));
					break;
	...
	private String formatColumn(int available, String value) {
	...
	public void dumpColumnNames(ArrayList<Column> columns) {
		...
		for (Column column : columns) {
			debug("column(" + column.getName() + ", " + column.getType() + ", "
					+ column.getSize() + ")");
		}
	}
}
